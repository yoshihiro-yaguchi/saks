...
export const Update = () => {
  ...
  const urlParams = useParams<ParamParseKey<typeof TRANSACTION_PATHS.UPDATE>>()

  // 画面ロード時処理
  useEffect(() => {
    dispatch(operations.init(urlParams.transactionId!))
  }, [])
  ...
  const detailRows: DetailRow[] = useAppSelector(
    (s: RootState) => s.updateTransaction.detailRows
  )

  // 税情報
  const taxInfos: Array<TaxInfo> = useAppSelector(
    (s: RootState) => s.updateTransaction.taxInfos
  )
  ...
  const modalState: ModalState = useAppSelector(
    (s: RootState) => s.updateTransaction.modal
  )

  // 法人区分が法人である
  const isCorporation =
    transactionHead.corporationDivision === constants.CORPORATION_CORPORATE
                ...
                <Table size="small" sx={{ width: "350px" }}>
                  <TableBody>
                    <StyledTableRow>
                      ...
                      <StyledTableRowCell sx={{ textAlign: "right" }}>
                        <Typo>
                          &yen;{transactionHead.subtotal.toLocaleString()}
                        </Typo>
                      </StyledTableRowCell>
                    </StyledTableRow>
                    <StyledTableRow>
                      ...
                      <StyledTableRowCell sx={{ textAlign: "right" }}>
                        <Typo>
                          &yen;{transactionHead.taxInclude.toLocaleString()}
                        </Typo>
                      </StyledTableRowCell>
                    </StyledTableRow>
                    <StyledTableRow>
                      ...
                      <StyledTableRowCell sx={{ textAlign: "right" }}>
                        <Typo>
                          &yen;{transactionHead.total.toLocaleString()}
                        </Typo>
                      </StyledTableRowCell>
                    </StyledTableRow>
                  </TableBody>
